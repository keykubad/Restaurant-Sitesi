...
function ustmenuler($basla,$limit){
	$ust	=mysql_query("select * from menuler where  menu_aktif='1' LIMIT ".$basla.",".$limit."");
		...
		//bu kısımda menu yapısına göre alt menu varsa bir sorgu yoksa baska sorgu
			...
			$ustid=$don['id'];
			echo '<li><a href="'.seo($menu_adi=$don["menu_adi"]).'-'.$ustid.'-menu.html">'.$menu_adi=$don["menu_adi"].'</a>';
...
function slider(){
	$slider	=mysql_query("select * from slider");
		while($s=dizi($slider)){
		  ...
		  echo'<li>';  
			echo '<a class="thumb" href="'.URL.''.$sresim=$s["slider_resim"].'">';
			echo '<img src="'.URL.''.$sresim=$s["slider_resim"].'" width="320" height="403"/> <span>'.$sek=$s["slider_ek"].'</span></a>';
...
lisans('454CF-FC1D1-C8896-C76D8-660DE'); 
?>